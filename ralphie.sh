...
plan_is_semantically_actionable() {
    ...
    if ! is_true "$has_validation" && grep -qiE '^[[:space:]]*-[[:space:]]\[[ x]\][[:space:]].*\b(validation|verify|verification|acceptance|success criteria|definition of done|readiness|qa|testing|test|smoke|gate)\b' "$plan_file" 2>/dev/null; then
        has_validation=true
    fi
